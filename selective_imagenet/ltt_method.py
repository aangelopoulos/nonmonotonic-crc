...
def apply_ltt_to_imagenet(cal_smx, cal_labels, val_smx, val_labels, 
    ...
    # Filter lambdas to ensure sufficient data
    lambdas_ltt = np.array([lam for lam in lambdas if nlambda(lam) >= 30])
    ...
    lhat = 1.0
    for lam in np.flip(lambdas_ltt):
        if selective_risk_ub(lam - 1/lambdas_ltt.shape[0]) > alpha: 
            lhat = lam
            break
